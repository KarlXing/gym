...
class CarRacing8(gym.Env, EzPickle):
    metadata = {
    ...
    def _create_track(self):
        CHECKPOINTS = 12
        random_track = int(self.np_random.uniform(0, 1) * self.num_tracks)  # only limited number of possible tracks
        random_num = 1.0 * random_track / self.num_tracks
        # Create checkpoints
        checkpoints = []
        for c in range(CHECKPOINTS):
            # alpha = 2*math.pi*c/CHECKPOINTS + self.np_random.uniform(0, 2*math.pi*1/CHECKPOINTS)
            alpha = 2*math.pi*c/CHECKPOINTS + random_num*(2*math.pi*1/CHECKPOINTS)
            # rad = self.np_random.uniform(TRACK_RAD/3, TRACK_RAD)
            rad = TRACK_RAD/3 + random_num * TRACK_RAD/3*2
            if c==0:
                alpha = 0
                rad = 1.5*TRACK_RAD
            if c==CHECKPOINTS-1:
